...
export default function Pricing() {
  ...
  const handleManualPayment = async () => {
    try {
      const res = await API.post('/payment/paypal-success', {
        orderID: 'TEST_ORDER_' + Date.now(),
        payerID: 'TEST_PAYER_123',
        planType: selectedPlan,
        amount: price
      });
      alert(`Payment Successful! ${res.data?.message || 'Updated!'}`);
    } catch (err) {
      console.error('Payment processing failed:', err);
      alert('Payment processing failed. Ensure backend is running.');
    }
  };
